...
const TherapistHero = (props) => {
    ...
    return (
        <div className="flex gap-5 flex-col items-center md:items-start md:flex-row md:justify-evenly p-5 border-b-2">

            <div>
                <img src={props.user.profilePic} className="w-96" alt="" />
            </div>

            <div className="md:w-[60%] lg:w-[50%] border-2 p-5 rounded-lg">

                <div className="therapist-heading my-5 lg:mb-10">
                    <div className="relative">
                        <h2 className="font-bold text-xl text-[#333]">{props.user.name}</h2>
                        <div className="absolute right-0 top-0 flex items-baseline">{printRatings(4)} &nbsp; 4/5</div>
                    </div>
                    <span className="text-[#555]">PHD in Therapy</span>
                </div>

                <div className="therapist-body lg:mt-10">
                    <ul>
